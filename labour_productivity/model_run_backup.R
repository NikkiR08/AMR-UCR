...
temp_sc2ecoli <- sc2_ecoli %>%
  ...
  mutate(annual_GDP_sc2ecoli = Y,
         ...
         total_Ypc_sc2ecoli = sum(Y_PC, na.rm=TRUE)) %>%
  select(c("country","year",          
           "annual_GDP_sc2ecoli" ,"annual_GDP_percapita_sc2ecoli",
           "total_Y_sc2ecoli" ,"total_Ypc_sc2ecoli")) %>%
  group_by(country) %>% 
  slice_max(year, n = 1) %>%
  as.data.table()
...
temp_sc1saureus <- sc1_saureus %>%
  ...
  mutate(annual_GDP_sc1saureus = Y,
         ...
         total_Ypc_sc1saureus = sum(Y_PC, na.rm=TRUE)) %>%
  select(c("country","year",          
           "annual_GDP_sc1saureus" ,"annual_GDP_percapita_sc1saureus",
           "total_Y_sc1saureus" ,"total_Ypc_sc1saureus")) %>%
  group_by(country) %>% 
  slice_max(year, n = 1) %>%
  as.data.table()
...
temp_sc2saureus <- sc2_saureus %>%
  ...
  mutate(annual_GDP_sc2saureus = Y,
         ...
         total_Ypc_sc2saureus = sum(Y_PC, na.rm=TRUE)) %>%
  select(c("country","year",          
           "annual_GDP_sc2saureus" ,"annual_GDP_percapita_sc2saureus",
           "total_Y_sc2saureus" ,"total_Ypc_sc2saureus")) %>%
  group_by(country) %>% 
  slice_max(year, n = 1) %>%
  as.data.table()
...
temp.all[ , total_Y_loss_sc2saureus := total_Ypc_basecase-total_Y_sc2saureus]
...
theMap2 <- mapCountryData( joinData, nameColumnToPlot="total_Y_loss_sc2saureus", addLegend=FALSE )
...
theMap4 <- mapCountryData( joinData, nameColumnToPlot="percentage_Y_loss_sc2saureus", addLegend=FALSE )
